...
```text
You are working in the current AquaLogic repository. Implement Goal 4 only: the
owner-approved retired tank lifecycle.

Read AGENTS.md, docs/INDEX.md, docs/DEVELOPMENT_STATUS.md, docs/DECISIONS.md, the
final-hardening hub, packets 01, 02, 04, 08, and 09, relevant Phase 01/04/05
specs, backend/web area guides, canonical domain/API/workflow docs, and git
status. Confirm that actuator uncertain-outcome safety and deletion cleanup are
implemented or explicitly account for their dependency before editing.

Inspect every route, service, model, schema, query, UI surface, and test that can
list, mutate, ingest for, publicly project, analyze, control, or permanently
delete a tank. Produce a file-level plan first covering:
- Alembic migration and active-row backfill;
- retired_at, retiring administrator, and bounded optional note;
- dedicated idempotent admin-only retirement endpoint and audit event;
- centralized active-tank write guard;
- public shutdown and transactional registered-device deactivation;
- blocking retirement during executing or uncleared unknown actuator work;
- active/retired/all authenticated query behavior;
- retained historical detail/analytics/media;
- read-only retired UI and removal from live fleet/control workflows;
- permanent deletion only after retirement;
- monitoring-expectation/incident interaction required by packet 09;
- backend/web/migration/documentation validation.

Implement the plan end to end. Retirement is one-way in this pass; do not add
reactivation. Preserve historical ownership and records. Do not automatically
erase firmware schedules, reassign devices, or turn this into generalized asset
management. Use stable domain errors and central guards rather than scattered
ad hoc checks.

Run focused lifecycle, permissions, public, device, actuator, analytics,
integrity, media, and web tests, then full backend tests, Alembic validation, and
web typecheck/tests/build. Update domain, API, architecture, workflows,
development status, and affected phase specs. Finish with changed files,
migration/backfill evidence, validation results, acceptance criteria, and any
